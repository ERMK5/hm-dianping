...
public class ShopServiceImpl extends ServiceImpl<ShopMapper, Shop> implements IShopService {
    ...
    public List<Shop> queryShopByType(Integer typeId, Integer current, Double x, Double y) {
        ...
        // 2. 计算分页参数
        int from = (current - 1) * SystemConstants.DEFAULT_PAGE_SIZE;// from：这一页要跳过多少条
        int end = current * SystemConstants.DEFAULT_PAGE_SIZE;

        // 3. 查询Redis，按照距离排序，分页。结果：shopId，距离distance
        // GEOSEARCH key BYLONLAT x y BYRADIUS 半径值(5000米) WITHDISTANCE
        ...
        if (list.size() <= from) {
            // 跳过的数大于数据数量，没有下一页了，结束
            return Collections.emptyList();
        }
        // 4.1 截取from到end的部分（分页查）
        List<Long> ids = new ArrayList<>(list.size());// 存查到的店铺id
        Map<String, Distance> distanceMap = new HashMap<>(list.size());// key是店铺id，value是距离
