...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public void sign() {
        ...
        // 5. 写入Redis的BitMap（底层是String）SETBIT key offset 1
    ...
    public Integer signCount() {
        ...
        // 4. 获取今天是本月的第几天
        int dayOfMonth = now.getDayOfMonth();
        // 5. 获取本月截止今天为止的所有签到记录（今天是几号就查几位），返回的是一个十进制数
        // BITFIELD key GET u[dayOfMonth] offset
        ...
        Long num = result.get(0);// 取出查询的签到结果
        if (num == null || num == 0) {
            return 0;
        }
        ...
        int count = 0;
        while (true) {
            // 让这个数字与1做与运算，这样得到数字的最后一个bit位，判断这个位置是0还是1
            if ((num & 1) == 0) {
                // 为0表示未签到，统计结束
                break;
            } else {
